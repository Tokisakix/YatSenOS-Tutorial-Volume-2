...
impl AtaDrive {
    ...
    fn humanized_size(&self) -> (f32, &'static str) {
        ...
        let bytes: f32 = (size * count) as f32;

        if bytes < 1000f32 {
            (bytes, "  B")
        } else if (bytes / (1 << 10) as f32) < 1000f32 {
            (bytes / (1 << 10) as f32, "KiB")
        } else if (bytes / (1 << 20) as f32) < 1000f32 {
            (bytes / (1 << 20) as f32, "MiB")
        } else {
            (bytes / (1 << 30) as f32, "GiB")
        }
    }
}
